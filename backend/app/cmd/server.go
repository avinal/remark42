...
type NotifyGroup struct {
	...
	UserTypes  []string `long:"user_types" env:"USER_TYPES" description:"types of user notifications" choice:"none" choice:"email" choice:"telegram" default:"none" env-delim:","`                                                                //nolint
	AdminTypes []string `long:"admin_types" env:"ADMIN_TYPES" description:"types of admin notifications" choice:"none" choice:"telegram" choice:"email" choice:"slack" default:"none" env-delim:","`                            //nolint
	...
	Email struct {
		...
		AdminNotifications  bool   `long:"notify_admin" env:"ADMIN" description:"[deprecated, use --notify.admin_types=email] notify admin on new comments via ADMIN_SHARED_EMAIL"`
	} `group:"email" namespace:"email" env-namespace:"EMAIL"`
	Slack struct {
...
func (s *ServerCommand) HandleDeprecatedFlags() (result []DeprecatedFlag) {
	...
	if len(s.Notify.Type) != 0 && (len(s.Notify.UserTypes) != 0 || len(s.Notify.AdminTypes) != 0) {
		s.handleDeprecatedNotifications()
		result = append(result, DeprecatedFlag{Old: "notify.type", New: "notify.(user|admin)_types", Version: "1.9"})
	}
	if s.Notify.Email.AdminNotifications && !contains("email", s.Notify.AdminTypes) {
		s.Notify.AdminTypes = append(s.Notify.AdminTypes, "email")
		result = append(result, DeprecatedFlag{Old: "notify.email.notify_admin", New: "notify.admin_types=email", Version: "1.9"})
	}
...
func (s *ServerCommand) handleDeprecatedNotifications() () {
	for _, t := range s.Notify.Type {
		if t == "email" && !contains(t, s.Notify.UserTypes) {
			s.Notify.UserTypes = append(s.Notify.UserTypes, t)
		}
		if (t == "telegram" || t == "slack") && !contains(t, s.Notify.AdminTypes) {
			s.Notify.AdminTypes = append(s.Notify.AdminTypes, t)
		}
	}
}
...
func (s *ServerCommand) newServerApp() (*serverApp, error) {
	...
	if contains("email", s.Notify.UserTypes) {
		emailNotifications = true
	}

	if contains("telegram", s.Notify.UserTypes) {
...
func (s *ServerCommand) makeNotify(dataStore *service.DataStore, authenticator *auth.Service) (*notify.Service, error) {
	...
	for _, t := range s.Notify.AdminTypes {
		switch t {
		case "slack":
	...
	for _, t := range s.Notify.UserTypes {
		switch t {
		case "telegram":
		case "email":
		case "none":
	...
	if contains("telegram", s.Notify.UserTypes) || contains("telegram", s.Notify.AdminTypes) {
		if contains("telegram", s.Notify.AdminTypes) && s.Notify.Telegram.Channel == "" {
			return nil, errors.New("--notify.telegram.channel must be set for admin notifications to work")
		}
		tg, err := notify.NewTelegram(s.Telegram.Token, s.Notify.Telegram.Channel, s.Telegram.Timeout, contains("telegram", s.Notify.UserTypes), s.Notify.Telegram.API)
		if err != nil {
	...
	if contains("email", s.Notify.UserTypes) || contains("email", s.Notify.AdminTypes) {
		...
		if contains("email", s.Notify.AdminTypes) {
	...
	if len(destinations) > 0 {
		log.Printf("[INFO] make notify, for users: %s, for admins: %s", s.Notify.UserTypes, s.Notify.AdminTypes)
